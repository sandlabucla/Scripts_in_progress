...
#Upload data
dataframe<-read.csv("/Users/aayushpatel/Desktop/SB_wave2_All_ages_Block2_CLEANFAKEDATA (Silvers).csv", header = TRUE)
  ...
  dataframe$NtBS_1R <- ifelse(dataframe$NtBS_1 == 1, dataframe$NtBS_1R <- 5,
                              ifelse(dataframe$NtBS_1 == 2, dataframe$NtBS_1R <- 4,
                                     ifelse(dataframe$NtBS_1 == 3, dataframe$NtBS_1R <- 3,
                                            ifelse(dataframe$NtBS_1 == 4, dataframe$NtBS_1R <- 2, 
                                                   ifelse(dataframe$NtBS_1 == 5, dataframe$NtBS_1R <- 1, 0)))))
  dataframe$NtBS_3R <- ifelse(dataframe$NtBS_3 == 1, dataframe$NtBS_3R <- 5,
                              ifelse(dataframe$NtBS_3 == 2, dataframe$NtBS_3R <- 4,
                                     ifelse(dataframe$NtBS_3 == 3, dataframe$NtBS_3R <- 3,
                                            ifelse(dataframe$NtBS_3 == 4, dataframe$NtBS_3R <- 4,
...
#Scores NtBS questionnaire using the sum of the 
NtBS_score_sum <- rowSums(dataframe[,paste("NtBS",c("1R", "2", "3R", "4", "5", "6", "7R", "8", "9", "10"),sep="_")], na.rm = FALSE)
NtBS_score_mean <- rowMeans(dataframe[,paste("NtBS",c("1R", "2", "3R", "4", "5", "6", "7R", "8", "9", "10"),sep="_")], na.rm = FALSE)
